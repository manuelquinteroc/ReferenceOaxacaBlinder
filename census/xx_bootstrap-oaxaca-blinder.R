...
# Configure ---------------------------------------------------------------
# Restrict to subsets large enough for stable bootstrap SEs and
# effects large enough to be potentially meaningful
fits_sized = fits_obs |>
...
# A sign flip occurs when the explained (or unexplained) component changes sign
# across the two reference groups: product < 0 means opposite signs; product == 0
# means one is exactly zero, which is treated as a flip candidate too
...
# Bootstrap only the sign-flip cases (much smaller than the full design in 01)
fits = mutate(design,
              fit = pmap(design,
                         ob$twoway_subset,
                         data   = acs,
