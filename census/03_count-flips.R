...
# Read Data ---------------------------------------------------------------
# nonlinear_fits: point estimates; nonlinear_boots: one row per bootstrap replicate
fits  = read_fst(here('census', 'temp', 'nonlinear_fits.fst')) |> as_tibble()
boots = read_fst(here('census', 'temp', 'nonlinear_boots.fst')) |> as_tibble()


# Standard Errors ---------------------------------------------------------
# Pivot to long so each (group key, statistic) gets its own SE from the bootstrap distribution
boot_se = boots |>
...
flips_components = fits |>
  # Drop cells too small for reliable inference or with negligible raw gaps
  ...
  left_join(boot_se) |>
  # stat_name is e.g. "explained_0" or "unexplained_1"; split into component + ref_pop
  separate(stat_name, into = c('component', 'ref_pop'), sep = '_') |>
  # Two-sided p-value under normal approximation using bootstrap SE
  mutate(p_value = 2 * pnorm(-abs(stat_value), sd = stat_se)) |>
  group_by(subset_name, subset_value, pop_name, y_name, algo_name, component) |>
  summarize(
    # product < 0 iff the two reference-group estimates have opposite signs
    is_flip     = prod(stat_value) < 0,
    # smallest p-value across ref groups: a flip is "significant" when at least
    # one reference-group estimate is individually distinguishable from zero
...
# A case counts as a flip if *either* component (explained or unexplained) flips
...
# Stack per-component rows with the "either" aggregate row before writing
