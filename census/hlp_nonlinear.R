# Nonlinear generalization of the Oaxaca-Blinder decomposition.
# Each algorithm (gbt, ols, glm, net) must return list(y_type, fit) and be
# handled by predict_mean / predict_glm / predict_brl.
...
prepare_data = function(data, pop_name, y_name, x_names = NULL) {
  ...
  if (is_null(x_names)) {
    features = data |>
      ...
      # one-hot encode categoricals; numeric covariates pass through as-is
  ...
  # Missing-indicator trick: add a 0/1 flag column, then zero out the NAs so
  # tree/linear models can train without NA propagation
  ...
  list(pop      = data[[pop_name]],
       y        = data[[y_name]],
       features = features,
       # folds used for CV-based hyperparameter tuning inside each algo
...
assign_folds = function(pop, y, seed = 1) {
  ...
  # stratify by both group membership and outcome so each fold is balanced
...
# Notation: mu_x{pop}_yx_{ref} = E[Y | X ~ pop, f ~ ref].
# explained_r   = counterfactual gap when reference group is r.
# unexplained_r = residual gap (structural difference) under reference r.
# A sign flip occurs when explained_0 * explained_1 < 0.
...
aligned_slope_intercept = function(data, y_name, pop_name, subset_name, subset_value) {
  ...
  # Assumption 5.1: both explained components share the same sign, and that
  # sign matches the intercept gap (sufficient condition against sign flips)
...
# Tune and train gradient-boosted trees via CV early stopping.
# num_threads = 1L prevents contention with outer furrr parallel workers.
...
ols = function(x, y, folds) {
  ...
  # folds != 0 selects all rows (folds are 1-3); no held-out test fold for OLS
...
predict_glm = function(model, x) {
  ...
  # drop aliased (NA) coefficients before matrix multiply
  cf = cf[!is.na(cf)]

  cf_names = names(cf)
  # strip the intercept name; we prepend the 1 column manually below
...
predict_brl = function(model, x) {
  y_hat = switch(model$y_type,
                 ...
                 # column 2 is P(Y=1) for binary brulee output
